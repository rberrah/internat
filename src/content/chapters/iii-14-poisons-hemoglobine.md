---
id: "iii-14-poisons-hemoglobine"
slug: "iii-14-poisons-hemoglobine"
title: "Poisons de l'hémoglobine : CO, plomb, méthémoglobinisants"
description: "Trois façons d'empêcher l'hémoglobine de transporter l'oxygène."
track: "section-III"
programItem: "III-14"
theme: "Toxicologie environnementale"
order: 314
duration: "12 min"
tags: ["toxicologie", "monoxyde de carbone", "plomb", "méthémoglobine"]
level: "intermediate"
sources: ["baud-garnier", "goldfrank", "centres-antipoison"]
reviewed_on: "2026-07-09"
quiz:
  - prompt: "Le monoxyde de carbone (CO) est toxique car il…"
    options:
      - "se lie à l'hémoglobine avec une affinité très supérieure à l'O2 (carboxyhémoglobine)"
      - "détruit les globules rouges"
      - "bloque le rein"
    correct: 0
  - prompt: "Le saturnisme (intoxication au plomb) provoque notamment…"
    options:
      - "une anémie et des troubles neurologiques"
      - "une cécité isolée"
      - "une hyperthyroïdie"
    correct: 0
  - prompt: "L'antidote d'une méthémoglobinémie symptomatique est…"
    options:
      - "le bleu de méthylène"
      - "la naloxone"
      - "le fomépizole"
    correct: 0
---

<!-- step:title="Pourquoi ce chapitre" -->
Trois toxiques différents convergent vers une même conséquence : **l'hémoglobine ne transporte plus l'oxygène**. Comprendre le mécanisme oriente le diagnostic et l'antidote.
<!-- /step -->

<!-- step:title="Monoxyde de carbone" -->
Le **CO** (chauffage défectueux, incendie) se lie à l'Hb avec une affinité **~200 fois** celle de l'O2 → **carboxyhémoglobine (HbCO)** : l'O2 n'est plus transporté ni délivré (hypoxie tissulaire).

- Clinique : céphalées, nausées, vertiges, troubles de conscience, coma ; **intoxication collective** évocatrice.
- Piège : la **SpO2 est faussement normale** (l'oxymètre ne distingue pas HbCO).

:::key
Diagnostic : **dosage de l'HbCO**. Traitement : **oxygénothérapie normobare** (100 %), voire **hyperbare** (formes graves, grossesse). Enquête sur la source (prévention).
:::
<!-- /step -->

<!-- step:title="Plomb (saturnisme)" -->
Le **plomb** (peintures anciennes, canalisations) inhibe des enzymes de la **synthèse de l'hème** :

- **anémie** (parfois avec hématies ponctuées) ;
- **troubles neurologiques** (encéphalopathie de l'enfant, neuropathie) ;
- douleurs abdominales (« coliques de plomb »), liseré gingival de Burton.

:::clinical
Diagnostic : **plombémie** ; le saturnisme infantile est une **maladie à déclaration obligatoire** (seuil **≥ 50 µg/L**). Traitement des formes sévères : **chélateurs** (EDTA calcique, DMSA) + éviction de la source.
:::
<!-- /step -->

<!-- step:title="Methemoglobinisants" -->
Certains toxiques (nitrites, nitrés, aniline, certains médicaments comme les sulfamides ou la dapsone) oxydent le fer de l'hème en **Fe³⁺** → **méthémoglobine**, incapable de fixer l'O2.

- Clinique : **cyanose** ne répondant pas à l'O2, sang « chocolat », dyspnée.

:::key
Antidote : **bleu de méthylène** (réduit la méthémoglobine) si taux élevé/symptomatique — **contre-indiqué en cas de déficit en G6PD** (inefficace et risque hémolytique).
:::
<!-- /step -->

<!-- step:title="A retenir" -->
- **CO** → HbCO (SpO2 faussement normale) : dosage HbCO + **O2 normo/hyperbare**.
- **Plomb** → blocage de la synthèse de l'hème : anémie + neuro ; plombémie, **DO**, chélation.
- **Méthémoglobinisants** → cyanose résistante à l'O2 : **bleu de méthylène**.
<!-- /step -->
