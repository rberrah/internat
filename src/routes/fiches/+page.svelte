<script>
  ...
  import { treesForChapter } from '$lib/content/decisionTrees';
...
<header class="head">
  ...
  <p class="lede">Une fiche par cours : le <strong>contexte</strong> en une phrase, l'<strong>essentiel à retenir</strong>, et l'<strong>arbre de décision</strong> quand le thème s'y prête.</p>
  <input class="search" type="search" placeholder="Rechercher une fiche (ex. anémie, diurétiques, acidose…)" bind:value={query} />
</header>
...
{#each tracks as t}
  ...
  {#if list.length}
    <details class="sec" open style={`--accent:${t.accent}`}>
      ...
      <div class="fiches">
        {#each list as c}
          {@const trees = treesForChapter(c.slug)}
          <article class="fiche">
            <div class="ftop">
            ...
            {#if trees.length}
              <details class="tree-toggle">
                <summary>🌳 Arbre de décision</summary>
                {#each trees as a}<div class="embed"><DecisionTree arbre={a} /></div>{/each}
              </details>
            {/if}
...
<style>
  ...
  .tree-toggle { margin-top: var(--space-3); }
  .tree-toggle > summary { font-family: var(--font-mono); font-size: var(--text-xs); color: var(--accent-ai); padding: 4px 0; }
  .embed { margin-top: var(--space-2); }
